...
`split_markdown.py` は大きなMarkdownファイルを指定されたサイズの小さなチャンクに分割するコマンドラインツールです。これは大きすぎるドキュメントの管理、大きなブログ記事の分割、または大きくなりすぎたMarkdownファイルの処理に役立ちます。

### 特徴

- 指定されたサイズに基づいてMarkdownファイルを分割
- カスタマイズ可能な出力ファイルの接頭辞
- 様々なサイズ単位（KB、MB、GB）のサポート
- 出力ディレクトリの指定オプション
- Linux風のコマンドライン引数
...
基本的な使い方：
```bash
./split_markdown.py -i 大きなファイル.md
```

すべてのオプションを使用：
```bash
./split_markdown.py -i 大きなファイル.md -s 500k -p 章_ -o 出力フォルダ
```
...
| 引数 | 長い形式 | 説明 | デフォルト |
|------|----------|------|------------|
| `-i` | `--input` | 分割する入力ファイル（必須） | なし |
| `-s` | `--size` | 分割サイズ（例：500k、1m、2g） | 1m |
| `-p` | `--prefix` | 出力ファイルの接頭辞 | part_ |
...
ファイルを500KBのチャンクに分割：
```bash
./split_markdown.py -i 文書.md -s 500k
```

ファイルを2MBのチャンクにカスタム接頭辞で分割：
```bash
./split_markdown.py -i 文書.md -s 2m -p セクション_
```

ファイルを分割して特定のディレクトリに保存：
```bash
./split_markdown.py -i 文書.md -o ./分割ファイル
```rkdown.py -i large_file.md -s 500k -p chapter_ -o output_folder
```
